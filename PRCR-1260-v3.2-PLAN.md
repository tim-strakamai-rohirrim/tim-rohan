...
| ~~3~~  | ~~`rohan_api`~~ | _Removed (v3.2) — MinIO files retained for traceability_                                                                                                                       |
...
- **Phases 2 and 4** can run in parallel — both add methods to `compliance.controller.ts` and `compliance.service.ts`. If done by the same developer, do them sequentially within a single PR. If split across developers, coordinate on merge order.
- **Phase 5** depends on Phases 2 + 4 (needs both endpoint contracts finalized). The frontend already supports `documentsToDelete` via the existing PATCH route — no backend changes needed for deletion.
...
2. **Phase 4** (per-doc auto-tag) — needed before Phase 8 but independent of 2
3. **Phase 2** (serve HTML) — depends on Phase 1
4. **Phase 5** (FE API + types) — depends on 2 + 4
5. **Phase 6** (HTML viewer) — biggest FE change, depends on 5
6. **Phase 7** (highlighting) — depends on 6
7. **Phase 8** (overview upload/delete) — depends on 4, 5; can be done in parallel with 6/7
8. **Phase 9** (cleanup) — after 6 is merged
9. **Phase 10** (E2E) — final
...
**Phase 3 — _Removed (v3.2)_**: MinIO files (original uploads and converted HTML) are retained for traceability. No storage cleanup is performed on document deletion. The existing `updateProject` deletion path handles DB cascade and `TaggableDocument` cleanup, which is sufficient.
